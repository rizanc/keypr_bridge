...
//import com.cloudkey.pms.micros.og.name.{CompanyCompanyType, NameAddress, Customer}
...
//  @Inject
//  private var defaultCurrency: Currency = _
//
...
//
//      resGuest.getProfiles.find(_.getCustomer != null).foreach(profile => {
//        val customer = profile.getCustomer
//
//        resv.setCustomerProfile(
//          new CustomerProfile(
...
//            profile.getPhones.filter(_.getPhoneType == PhoneNumberRole.PHONE).map(_.getPhoneNumber),
//            profile.getPhones.filter(_.getPhoneType == PhoneNumberRole.FAX).map(_.getPhoneNumber)
//          )
//        )
//      })
//
//      findProfile(resGuest, CompanyCompanyType.COMPANY).foreach(profile => {
//        val company = profile.getCompany
//
//        resv.setTravelAgentProfile(
//          new TravelAgentProfile(
//            IdUtils.findInternalProfileId(profile.getProfileIDs).orNull,
...
//      })
//
//      findProfile(resGuest, CompanyCompanyType.GROUP).foreach(profile => {
//        val company = profile.getCompany
//
//        resv.setTravelAgentProfile(
//          new TravelAgentProfile(
...
//      )
//
//      val creditCardOpt = Option(roomStay.getGuarantee)
...
//
//      resv.setRoomRates(roomStay.getRoomRates.headOption.flatMap(_.getRates.asScala).map(convertRate).toList)
//
...
//  }
//
//  private def findProfile(resGuest: ResGuest, profileType: CompanyCompanyType) = {
//    resGuest.getProfiles.find(profile => profile.getCompany != null && profile.getCompany.getCompanyType == profileType)
//  }
//
...
//    val counts = Option(roomStay.getGuestCounts)
