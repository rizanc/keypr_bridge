...
public class MicrosPMSMessageParser implements IParserInterface {
	...
	public CheckInResponse guestCheckIn(CheckInRequest checkInRequest) {
		...
		MicrosPMSLogger.logInfo(MicrosPMSMessageParser.class, " guestCheckIn "," Get Response from OXI Simulator in xml format :: "
				+ xmlResponse);
	...
	public SearchReservationResponse searchReservationData(
		...
		if(futureBookingSummaryRequest != null && (futureBookingSummaryRequest.getFirstName().length() > 0 
				...
				|| futureBookingSummaryRequest.getCreditCard().length() > 0)){
...
MicrosPMSLogger.logInfo(MicrosPMSMessageParser.class,
	...
	private FutureBookingSummaryRequest getFutureBookingRequestObject(
		...
		String creditCardNumber = futureBookingSummaryRequest.getCreditCard();
		...
		if (confirmationNumber != null) {
			...
			objUniqueID.setSource(confirmationNumber);
	...
	private com.micros.pms.bean.CheckInRequest getCheckInRequestObject(
		...
		com.micros.pms.factory.ObjectFactory objectFactory = new com.micros.pms.factory.ObjectFactory();
	...
	private com.micros.pms.bean.CheckOutRequest getCheckOutRequestObject(
		...
		/*To set the card information with the request base into checkout request.*/
	...
	private UpdateBookingResponse getModifyBookingResponseObject(
		...
		for (com.micros.reservation.ReservationServiceStub.UniqueID objId : arrUniIds) {
			...
			if (objId.getType().toString().equalsIgnoreCase("INTERNAL")) {
				objReservation.setConfirmationNumber(objId.getSource());
